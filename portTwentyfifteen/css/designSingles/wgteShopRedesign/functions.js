    ...
    jQuery(function(){
        ...
        function displayTemplate(currentDraggable, pageID, contentArray){
            ...
            var template = {
                'page-id-37':function(){
                    ...
                    var html = '<li class="main-content-listItem about-listItem"><img class="about-img" src=' + contentArray["featured_imageLink"] + ' alt=' + contentArray["featured_imageTitle"] + '/><div class="content">' + contentArray["textContent"] + '</div></li>';
                    return html;
                },
                'page-id-94':function(){
                    console.log("Using about template");
        ...
        function checkOrReplaceUrl(res, currentDraggable, currentDraggableTitle){
            ...
            */
            ...
            else if((jQuery("body").hasClass("page-id-117") || jQuery("body").hasClass("page-id-115") || jQuery("body").hasClass("home")) && (hashFragments.length >= 4)){
                //delete elements of hashFragments
                hashFragments.splice(0,0);
                //replace necessary hashFragments
                hashFragments[2] = currentDraggableString;
                hashFragments.splice(3, 1);
                console.log(hashFragments);
                //stitch url 
                var newStateHash ="";
                for(var i =0; i < hashFragments.length; i++){
        ...
        function loadContent(event, ui) {
            ...
            //single page loads
            if((body.hasClass("single")) && ((event.type =="drop") || event.type =="click")){
...
*/
    ...
    jQuery(document).on("click", '.post-preview-link', function(event){
        ...
        if (link.hasClass("mobile-preview-link") && (jQuery("body").width() >= 768)){
            ...
            docReady.text = 'jQuery(document).ready(function(){ jQuery("#highlights").slippry(); window.setTimeout(function(){ jQuery(".shop_item_list_wrap").mCustomScrollbar({ theme:"minimal-dark", axis:"x", alwaysShowScrollbar:2}); }, 1 * 100); });';
   ...
   /* jQuery(function(){
    	console.log("resize cats");
    	if(jQuery(window).width() <= 1023){
    		jQuery('.shop_item_img').each( function(){
    			jQuery(this).attr("src", jQuery(this).attr("src").replace("medium", "small"));
    		});
    	}
    });
    jQuery(window).resize(function(){
    	if(jQuery(window).width() <= 1023){
    		...
    		jQuery('.shop_item_img').each(function(){
    			jQuery(this).attr("src", jQuery(this).attr("src").replace("small", "medium"));
    		});	
    	}    	
    }); */
});
